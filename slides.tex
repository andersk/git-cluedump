...
\begin{frame}
  ...
  \item[\texttt{git mv \textit{oldfile} \textit{newfile}}] Shortcut
    for \texttt{mv \textit{oldfile} \textit{newfile}} plus the
    appropriate additions and removals in the index.
...
\begin{frame}
  ...
  \item[\texttt{\textit{commit}:\textit{filename}}] The given file or
    directory inside \texttt{\textit{commit}}'s tree.
  \end{description}
...
\begin{frame}
  ...
  \item[\texttt{git cherry-pick \textit{commit}}] Constructs a new
    commit on \texttt{HEAD} that performs the same changes as
    \texttt{\textit{commit}}.
  \item[\texttt{git revert \textit{commit}}] Constructs a new commit
    on \texttt{HEAD} that performs the reverse of the changes in
    \texttt{\textit{commit}}.
  \end{description}
